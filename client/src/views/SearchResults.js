    ...
    return (
        <React.Fragment>
            ...
            <div className="result-container">
                {tab === 'Movies' ? (
                    <>{movies.length > 0 ? movies.map((movie) => <MovieCard movie={movie} key={movie.id} />) : <p>No movies found</p>}</>
                ) : tab === 'TV shows' ? (
                    <>{tv.length > 0 ? tv.map((tv) => <TVCard tv={tv} key={tv.id} />) : <p>No TV shows found</p>}</>
                ) : (
                    <>
                        {movies.length > 0 ? movies.map((movie) => <MovieCard movie={movie} key={movie.id} />) : <p>No movies found</p>}
